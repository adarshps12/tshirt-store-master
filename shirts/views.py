...
def signup(request, form=None):
    if (request.method == 'GET'):
        ...
        # data = UserUploadForm(request.FILES)
        if form.is_valid():
            user = form.save()
...
def upload(request):
    form1 = UserUploadForm()
    if (request.method == 'POST'):
        form1 = UserUploadForm(request.Files)
        if form1.is_valid():
            form1.save()
    return render(request, 'cart.html', {'form1':form1})
